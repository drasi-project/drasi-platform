...
use drasi_source_sdk::{stream, ChangeOp, ChangeStream, ReactivatorBuilder, ReactivatorError, SourceChange, SourceElement, StateStore};
use futures::TryStreamExt;
use k8s_openapi::{api::{apps::v1::{DaemonSet, Deployment, ReplicaSet, StatefulSet}, batch::v1::Job, core::v1::{Node, PersistentVolume, PersistentVolumeClaim, Pod, Service, ServiceAccount}, networking::v1::Ingress}, Metadata, Resource};
use kube::{
    api::{Api, ResourceExt, WatchParams}, config::{KubeConfigOptions, Kubeconfig}, runtime::WatchStreamExt, Client, Config
};
...
async fn my_stream(_context: (), state_store: Arc<dyn StateStore + Send + Sync>) -> Result<ChangeStream, ReactivatorError> {
    ...
    let kube_config = {
        match env::var("kubeConfig") {
            Ok(config) => {
                ...
                Config::from_custom_kubeconfig(kc, &KubeConfigOptions::default()).await.unwrap()
            },
            Err(_) => {
                Config::infer().await.unwrap()
            },
        }
    };
    ...
    _ = spawn_monitor_task::<PersistentVolume>(tx.clone(), kube_client.clone(), state_store.clone());
    _ = spawn_monitor_task::<PersistentVolumeClaim>(tx.clone(), kube_client.clone(), state_store.clone());
...
where  
    ...
    K: kube::Resource + Metadata + std::clone::Clone + std::marker::Send + for<'de> serde::Deserialize<'de> + std::fmt::Debug + serde::Serialize + 'static
{
                        ...
                        Some(wevt) => {
                            match wevt {
                                ...
                                kube::api::WatchEvent::Bookmark(bookmark) => {
                                    log::info!("received bookmark {} for {}", bookmark.metadata.resource_version, K::KIND);
                                    cursor = bookmark.metadata.resource_version.clone();
                                    if let Err(err) = state_store.put(K::KIND, cursor.to_string().as_bytes().to_vec()).await {
                                ...
                                kube::api::WatchEvent::Error(error_response) => {
                                    ...
                                    match error_response.code {
                                        ...
                                        _ => {
                                            log::error!("received error response for {}: {:?}", K::KIND, error_response);
                                            tokio::time::sleep(tokio::time::Duration::from_secs(err_backoff_secs)).await;
                                            break;
                                        },
                                    }
