...
{
    internal class PodWatcher : BackgroundService, IStateFetcher
    {
        ...
        public PodWatcher(Kubernetes kubeClient, IChangePublisher publisher, IStateStore stateStore, IGraphChangeMapper mapper)
        {
        ...
        {
            Console.WriteLine("Watching for pod changes...");
            while (!stoppingToken.IsCancellationRequested)
            {
                var resp = _kubeClient.CoreV1.ListPodForAllNamespacesWithHttpMessagesAsync(watch: true);

                await foreach (var (type, item) in resp.WatchAsync<V1Pod, V1PodList>())
                {
                    try
                    {
            ...
            Console.WriteLine("Pods watcher stopped");
        }
        ...
        public async Task<(IEnumerable<VertexState> Nodes, IEnumerable<EdgeState> Edges)> GetCurrent(string[] nodeLabels, string[] relLabels)
        {
            ...
            var resp = await _kubeClient.CoreV1.ListPodForAllNamespacesAsync();
