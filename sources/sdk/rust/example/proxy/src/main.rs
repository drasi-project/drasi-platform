    ...
    let stream = stream! {
        if req.node_labels.contains(&"Location".to_string()) {
            yield Ok(SourceElement::Node {
                ...
                properties: vec![
                    ("longitude".to_string(), Value::Number(Number::from_f64(50.1).expect("valid f64"))),
                    ("latitude".to_string(), Value::Number(Number::from_f64(60.7).expect("valid f64"))),
                ].into_iter().collect(),
            });
            ...
            yield Ok(SourceElement::Node {
                ...
                labels: vec!["Location".to_string()],
                properties: vec![
                    ("longitude".to_string(), Value::Number(Number::from_f64(58.9).expect("valid f64"))),
                    ("latitude".to_string(), Value::Number(Number::from_f64(72.1).expect("valid f64"))),
                ].into_iter().collect(),
            });
        }
    };
