...
kubectl apply -f https://raw.githubusercontent.com/ruokun-niu/drasi-platform/smoke-test-test/dev-tools/smoke-tests/resources/smoke-test-postgres.yaml -n default
...
drasi apply -f https://raw.githubusercontent.com/ruokun-niu/drasi-platform/smoke-test-test/dev-tools/smoke-tests/resources/smoke-test-source.yaml
drasi wait source smoke-test -t 120

# Apply continuous query 
echo Applying a ContinuousQuery with the name 'smoke-query'
drasi apply -f https://raw.githubusercontent.com/ruokun-niu/drasi-platform/smoke-test-test/dev-tools/smoke-tests/resources/smoke-test-query.yaml
sleep 5

# Apply Reaction
echo Applying a Reaction with the name 'smoke-result-reaction'
drasi apply -f https://raw.githubusercontent.com/ruokun-niu/drasi-platform/smoke-test-test/dev-tools/smoke-tests/resources/smoke-test-reaction.yaml

drasi wait reaction smoke-result-reaction -t 120

# Initial result
initial_output=$(kubectl run curl-pod --image=curlimages/curl -n $namespace --restart=Never  --rm --attach -q -- sh -c 'curl -s http://smoke-result-reaction-gateway:8080/smoke-query/all' 2>/dev/null)
...
if [ "$final_parsed_output" == "$expected_output" ]; then
    ...
    echo "Resources are not deleted. If you wish to clean up everything, run 'curl -s https://raw.githubusercontent.com/ruokun-niu/drasi-platform/smoke-test-test/dev-tools/smoke-tests/cleanup-smoke-test.sh | bash'"
    exit 1
fi