...
const AbstractEndpoint_1 = require("./AbstractEndpoint");
const Network_consts_1 = require("./Network.consts");
const url_1 = require("url");
class GrpcEndpoint extends AbstractEndpoint_1.Endpoint {
    constructor(url) {
        super(url);
        this._authority = "";
        this._authority = Network_consts_1.URIParseConfig.DEFAULT_AUTHORITY;
        const parsedUrl = new url_1.URL(this.preprocessUri(url));
    ...
    preprocessUri(url) {
        ...
        if (urlList.length === 3 && !url.includes("://")) {
            ...
            !url.includes("://") &&
            Network_consts_1.URIParseConfig.ACCEPTED_SCHEMES_GRPC.includes(urlList[0])) {
            ...
            if (urlList.length === 1) {
                ...
                if (url[0] === ":") {
                    url = `${Network_consts_1.URIParseConfig.DEFAULT_SCHEME_GRPC}://${Network_consts_1.URIParseConfig.DEFAULT_HOSTNAME}${url}`;
                }
                else {
                    url = `${Network_consts_1.URIParseConfig.DEFAULT_SCHEME_GRPC}://${url}`;
                }
            }
            else {
                ...
                if (!Network_consts_1.URIParseConfig.ACCEPTED_SCHEMES_GRPC.includes(scheme)) {
    ...
    validatePathAndQuery(parsedUrl) {
        if (parsedUrl.pathname && parsedUrl.pathname !== "/") {
            throw new Error(`Paths are not supported for gRPC endpoints: '${parsedUrl.pathname}'`);
        }
        const params = new url_1.URLSearchParams(parsedUrl.search);
    ...
    setHostname(parsedUrl) {
        if (!parsedUrl.hostname) {
            this._hostname = Network_consts_1.URIParseConfig.DEFAULT_HOSTNAME;
            return;
        }
    ...
    setScheme(parsedUrl) {
        if (!parsedUrl.protocol) {
            this._scheme = Network_consts_1.URIParseConfig.DEFAULT_SCHEME_GRPC;
            return;
        }
    ...
    setPort(parsedUrl) {
        ...
        this._port = parsedUrl.port ? parseInt(parsedUrl.port) : Network_consts_1.URIParseConfig.DEFAULT_PORT;
    }
    setEndpoint() {
