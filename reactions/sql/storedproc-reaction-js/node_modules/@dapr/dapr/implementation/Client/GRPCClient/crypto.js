...
const Streams_util_1 = require("../../../utils/Streams.util");
class GRPCClientCrypto {
    constructor(client) {
        this.client = client;
    }
    async encrypt(arg0, opts) {
        // Handle overloading
        ...
        const duplexStream = new Streams_util_1.DaprChunkedStream(grpcStream, dapr_pb_1.EncryptRequest, (req) => {
            ...
            const reqOptions = new dapr_pb_1.EncryptRequestOptions();
    ...
    async decrypt(arg0, opts) {
        ...
        // Create a duplex stream that will send data to the server and read from it
        const duplexStream = new Streams_util_1.DaprChunkedStream(grpcStream, dapr_pb_1.DecryptRequest, (req) => {
            // Disable the @typescript-eslint/no-non-null-assertion linter in this block because opts here is guaranteed to be non-nil (see the check above), but TS doesn't seem to believe that
            /* eslint-disable @typescript-eslint/no-non-null-assertion */
            const reqOptions = new dapr_pb_1.DecryptRequestOptions();
            reqOptions.setComponentName(opts.componentName);
            if (opts.keyName) {
                reqOptions.setKeyName(opts.keyName);
            }
    ...
    processStream(duplexStream, inData) {
...
exports.default = GRPCClientCrypto;
